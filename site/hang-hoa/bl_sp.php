...
<?php
 ...
 foreach($show_bl as $bl){
  ...
  <tr>
            ...
            <tr>
				<td colspan="3"><button onclick="hienThi(form<?=$bl['id_binhluan']?>);" style="color: red;">Reply</button></td>
             
                <form action="" method="post" id="form<?=$bl['id_binhluan']?>" class="row replyRow" style="display: none;" >
                    <div class="form-group green-border-focus">
                        <input type="text" value="<?=$bl['id_binhluan']?>" name="reply">
                        <textarea class="form-control" name="n  oi_dung" id="exampleFormControlTextarea5" rows="3"></textarea>
                    </div>
...
function hienThi(visible){
   var phi = document.getElementById("form"+<?=$bl['id_binhluan']?>)
